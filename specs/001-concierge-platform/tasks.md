...
- [ ] T151 [C] Implement service-to-service Vault credential: modelserver and guardrails sidecar receive X-Service-Token header on every request from API; token issued from Vault and rotated per Vault policy. Integration test: unauthorized calls return 401 (FR-036). _(guardrails sidecar done: Vault-issued token + 401 enforcement + integration test `tests/integration/test_guardrails_service_token.py`; modelserver side blocked until `services/modelserver/app.py` exists.)_
